...
const { LLAMACPP_CHAT_URL, LLAMACPP_EMBED_URL, LLAMACPP_BIN, LLAMACPP_NGL, LLAMACPP_VARIANT, LLAMACPP_DEVICE, MODELS_DIR, DEFAULT_CONTEXT, EMBED_BATCH } = require('./config');
...
/* ---- the backend this process supervises ----
 *
 * Mutable, because the desktop shell no longer has to know the answer before
 * the server may start. Resolving the llama.cpp build can take minutes on a
 * first run (a 250 MB CUDA archive) and can take a surprisingly long time even
 * when it is already installed, and the whole app used to sit behind that: the
 * shell awaited it, then forked this server, then loaded the UI. Projects,
 * skills, settings and remote models were all hostage to a local backend the
 * user might not even be about to use.
 *
 * So the shell forks first and sends the resolved build down the IPC channel
 * when it has one (see configureBackend). Until then this module behaves
 * exactly as it does in headless `npm start`: it supervises nothing.
 *
 * Still seeded from the environment, because that is how a restart after a
 * Preferences change gets the answer back without another round trip. */
const backend = { bin: LLAMACPP_BIN, variant: LLAMACPP_VARIANT, device: LLAMACPP_DEVICE };

/* Whether this process spawns and respawns llama-server itself. A function,
 * not a constant, because the answer arrives after boot now. */
const autoManaged = () => Boolean(backend.bin);

/* What the shell is doing about the backend right now, for /api/health to
 * report and the status pill to show. A silent multi-minute wait is
 * indistinguishable from a hang — that was the whole problem. */
let setup = null;

/**
 * Adopt a backend the shell has resolved (and SHA256-verified), or record
 * what it is still doing about it. Called over IPC from electron/server.js.
 */
function configureBackend({ bin, variant, device } = {}) {
  if (bin) backend.bin = bin;
  if (variant) backend.variant = variant;
  if (device !== undefined) backend.device = device;
}

function setSetupState(state) { setup = state; }
function setupState() { return setup; }

/* The shell talks down the same channel this module already reports PIDs up.
 * Guarded because there is no channel at all under `npm start`. */
if (typeof process.on === 'function') {
  process.on('message', (msg) => {
    if (!msg || typeof msg !== 'object') return;
    if (msg.type === 'llamacpp:config') {
      configureBackend(msg);
      setSetupState(null);            // resolved — nothing left to narrate
    } else if (msg.type === 'llamacpp:setup') {
      setSetupState(msg.state || null);
    }
  });
}
...
/**
 * Batch/context size to launch the embed instance with, in tokens.
 *
 * EMBED_BATCH, clamped down to the embedding model's own trained context —
 * llama-server exits rather than starts if -c overshoots that, and a model
 * too small for a full chunk should still come up and embed the shorter
 * ones. Falls back to EMBED_BATCH when the header can't be read; an
 * unreadable GGUF is the model loader's problem to report, not this
 * function's, and guessing small here would quietly reintroduce the very
 * truncation this exists to prevent.
 */
async function embedBatchFor(modelPath) {
  try {
    const { contextLength } = await gguf.describeModel(modelPath, path.basename(modelPath));
    if (Number.isFinite(contextLength) && contextLength > 0) return Math.min(EMBED_BATCH, contextLength);
  } catch { /* fall through */ }
  return EMBED_BATCH;
}
...
function spawnInstance(inst, modelPath, numCtx, gpuLayers) {
  return (async () => {
    ...
    if (gpuLayers > 0 && backend.device) args.push('--device', backend.device);
    if (inst.role === 'chat') {
      args.push('-c', String(numCtx));
    } else {
      /* Mean pooling requires the whole sequence in one physical batch, and
       * llama.cpp defaults that to 512 tokens — smaller than a CHUNK_CHARS
       * chunk of dense text, which then fails to embed at all. Size the batch
       * to EMBED_BATCH, but never ask for more than the model was trained
       * for: llama-server refuses to start if -c exceeds a small embedder's
       * own context. A model too small to hold a chunk still embeds the
       * shorter ones rather than failing to launch. */
      const n = await embedBatchFor(modelPath);
      args.push('--embedding', '--pooling', 'mean',
                '-c', String(n), '-b', String(n), '-ub', String(n));
    }

    /* Strip this app's own settings — above all the OpenRouter key — from
     * what the runner inherits. llama-server has no use for any of them, and
     * a credential should not be sitting in the environment of a process
     * that doesn't need it.
     *
     * A denylist here where lib/mcp.js childEnv uses an allowlist, and the
     * difference is deliberate: an MCP server is arbitrary third-party code,
     * so it gets only what we can justify handing over. This binary is one
     * we pinned, downloaded and SHA256-verified ourselves, and it needs an
     * ambient environment we cannot fully enumerate — CUDA and Vulkan loader
     * variables, driver paths, the DLL search path. Naming what to remove is
     * honest about which of the two we actually understand. */
    const env = Object.fromEntries(
      Object.entries(process.env).filter(([k]) => !/^(MECHAPE|MONKII)_/i.test(k)));
...
/* Registered unconditionally now. These used to be gated on being the
 * supervising process at module load, which stopped being knowable then: the
 * shell hands the backend over later, so a process that owned nothing at boot
 * can own two llama-server instances a minute in. Cleaning up nothing is
 * free; failing to clean up leaves orphans. */
...
async function getVersion() {
  if (autoManaged()) {
    /* Missing here almost always means a scanner took it. llama-server.exe is
     * an unsigned upstream binary and it is the one antivirus actually
     * quarantines — sometimes minutes into a session, long after it was
     * unpacked, verified and used once. Saying so beats "unreachable", which
     * sends people looking at ports and firewalls for something sitting in a
     * quarantine folder. */
    if (!fs.existsSync(backend.bin)) {
      throw new Error('llama-server is missing from disk — antivirus most likely quarantined it');
    }
    return 'llama.cpp';
  }
...
function currentAccel() {
  ...
  return cpuOnly ? 'CPU' : accelLabel(backend.variant);
}
...
/**
 * One non-streaming turn that may come back asking for tools.
 *
 * Separate from chatOnce because it returns the whole assistant message
 * rather than just its text: the tool loop needs `tool_calls`, and the
 * message object has to be appended to the history verbatim so the model
 * sees its own request alongside the result.
 *
 * Tool calling is a llama-server extension to the OpenAI schema and needs a
 * model trained for it. A model that ignores `tools` simply answers in
 * prose, which the loop treats as "no tools wanted" — the degradation is
 * quiet and correct.
 */
async function chatWithTools({ model, messages, tools, timeoutMs = 180000, signal }) {
...
module.exports = {
  getVersion, currentAccel, listModels, showModel, streamChat, chatOnce, chatWithTools, embed,
  pullModel, deleteModel,
  configureBackend, setSetupState, setupState,
