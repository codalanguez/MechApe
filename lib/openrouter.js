...
/* How long a description may be before the catalog stops being a catalog.
 * Four hundred-odd models each carrying a paragraph is a payload nobody
 * asked for; two lines is what the row can show. */
const DESC_MAX = 220;

/**
 * One catalog entry, reduced to what MechApe actually uses. Pure, and
 * exported for tests.
 *
 * The capability flags are read straight out of the API rather than guessed
 * from the model's name, which matters: "can this model see an image" and
 * "can this model call a tool" are the two questions people actually pick a
 * remote model to answer, and both are structured fields. What is NOT here is
 * any notion of a model being good at "writing" or "general" work — OpenRouter
 * publishes no such field, and inferring one from marketing prose would dress
 * a guess up as a filter.
 *
 * The benchmark indices are the honest middle ground. Roughly half the
 * catalog carries them; the rest are left null so the UI can say "unrated"
 * instead of silently sorting them last as if they had scored zero.
 */
function mapModel(m) {
  ...
  const bench = (m.benchmarks && m.benchmarks.artificial_analysis) || {};
  ...
  return {
    ...
    // capabilities, as reported — not inferred from the name
    vision: inputs.includes('image'),
    tools: params.includes('tools'),
    reasoning: params.includes('reasoning'),
    // null, never 0, when a model carries no benchmarks at all
    coding: asNum(bench.coding_index),
    intelligence: asNum(bench.intelligence_index),
    agentic: asNum(bench.agentic_index),
    description: desc.length > DESC_MAX ? `${desc.slice(0, DESC_MAX - 1).trimEnd()}…` : desc,
  };
}
...
module.exports = {
  ...
  // exported for tests
  looksLikeNoToolSupport, mapModel,
};
