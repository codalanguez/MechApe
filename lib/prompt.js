...
/**
 * `includeMemory` is opt-in, and named for the decision rather than for the
 * backend, so the safe path is the one you get by saying nothing.
 *
 * Everything else in the prompt comes from this conversation or the project
 * the user deliberately opened with this model. Cross-chat memory is the one
 * part assembled from *other* conversations, so sending it to a remote
 * provider would be more than the user agreed to when they picked a remote
 * model. A caller that forgets the flag loses the feature; the inverse
 * default would leak a profile.
 */
async function buildSystem(project, extraSkillIds, chat, query, { includeMemory = false } = {}) {
  const sections = [];
  sections.push(
  ...
  /* Cross-chat memory, above skills and attachments: it is background about
   * who is asking, and should lose to anything more specific. Empty string
   * when memory is off or nothing is relevant, so a prompt without memories
   * is byte-identical to one built before this feature existed.
   *
   * Never on a remote chat. Picking a remote model consents to sending THIS
   * conversation to a provider — not to sending a profile distilled from
   * every other one. */
  if (includeMemory) {
    const memories = memory.promptBlock(query);
    if (memories) sections.push(`# Memory\n${memories}`);
  }
