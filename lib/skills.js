...
const { isNetworkPath, realpathish, containedIn } = require('./security');
...
async function generateSkill(name, description, model) {
  const { chatOnce } = require('./llamacpp'); // lazy: avoid loading at startup
...
function importSkill(srcPath, { force, asId } = {}) {
  ...
  (function tally(dir, depth) {
    ...
    for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      /* A link is not a skill file. fs.cpSync below runs with its default
       * dereference:false, so a folder containing `refs -> C:\Users\me\.ssh`
       * was copied across as a link, and a later
       * GET /api/skills/<id>/file?path=refs/id_rsa read straight through it.
       *
       * Refusing beats the alternatives: dereferencing would copy the private
       * key into the skills folder for real, and skipping silently produces a
       * skill missing files with nothing to explain why. Checked here in the
       * tally, which already runs before the rmSync below, so a rejected
       * import never destroys the skill it was replacing. */
      if (e.isSymbolicLink()) {
        throw new Error(`"${e.name}" is a shortcut or symbolic link — a skill must contain real files only`);
      }
...
function skillDetail(sid) {
  ...
  (function walk(d, rel, depth) {
    ...
    for (const e of fs.readdirSync(d, { withFileTypes: true })) {
      ...
      /* A junction or symlink is neither isDirectory() nor isFile() here —
       * Dirent comes from lstat — so it used to fall through to the else and
       * get listed as an ordinary reference file, with statSync following it
       * to report the *target's* size. Skipping keeps someone else's private
       * key out of the UI; skillFile refuses to read through a link on its
       * own, so this is presentation rather than the fence. */
...
function skillFile(sid, relPath) {
  ...
  /* Lexical rejections first, before any of this reaches the filesystem. A
   * relPath naming a network location resolves to a UNC path, and resolving
   * one on Windows authenticates to whoever chose the hostname before it
   * fails — see isNetworkPath in lib/security.js. Order is the whole point. */
  if (relPath.includes('\0') || isNetworkPath(relPath)) throw new Error('bad path');

  const dir = realpathish(path.join(SKILLS_DIR, sid));
  const target = path.resolve(dir, relPath);
  if (!containedIn(dir, target)) throw new Error('bad path');

  /* Then again on the resolved path. The check above is the string compare
   * this function used to rely on alone, and a string compare cannot see a
   * link: importSkill copies with fs.cpSync's default dereference:false, so a
   * skill that shipped a junction named `refs` pointing at C:\Users\me\.ssh
   * arrived as a junction, and refs/id_rsa passed the prefix test with room
   * to spare.
   *
   * The fence here is the skill folder itself, deliberately NOT pathAllowed:
   * SKILLS_DIR legitimately sits outside MECHAPE_FS_ROOTS — Preferences
   * suggests pointing it at ~\.claude\skills while file access is narrowed to
   * a projects drive — so gating this on the attachment allowlist would break
   * skill previews for exactly the people who tightened their settings.
   * Containment inside the resolved skill folder is the tighter fence for
   * this particular read. */
  const real = realpathish(target);
  if (!containedIn(dir, real)) throw new Error('that file points outside the skill folder');

  const { readTextFile } = require('./attachments');
  const text = readTextFile(real, 64 * 1024);
